...
- `syncNow` (action): `{ productionId }` — member perm; lists all hub folders (folderIds + shot folders), upserts assets by driveFileId (name/md5/size/trashed→missing), fetches Drive thumbnails (thumbnailLink bytes with hub token) into Convex storage when md5 changed or thumb missing; NEW unknown files → asset rows (unassigned, uploadedBy = hub connection's user). Patches syncedAt. Activity `drive.synced` once per manual sync ("Synced hub — 3 new files").
